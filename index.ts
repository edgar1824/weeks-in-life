...
function toggleError(id: string, show: boolean, text?: string) {
  const elem = document
...
form.addEventListener("submit", (e) => {
  ...
  const birth_date = new Date(data["birth-date"] as string);
  ...
  total.innerHTML = `Total weeks: ${[...Object.values(weeks)].reduce(
    (cur, week) => cur + week.length,
    0
  ...
  Object.entries(weeks).forEach(([key, weeks_arr], year_index) => {
    ...
    let start_col = col_index,
      end_col = (weeks_arr.length + col_index) % cols_length,
      start_row = row_index,
      end_row = Math.ceil((weeks_arr.length + gl_index) / cols_length);
    ...
    weeks_arr.forEach((week, i) => {
      ...
      // end of col
      if (
        (row_index + 1 === end_row && col_index <= end_col) ||
        (row_index === end_row - 2 &&
          col_index > end_col - 1 &&
          year_index === Object.entries(weeks).length - 1) ||
