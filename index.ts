...
form.addEventListener("submit", (e) => {
  ...
  while (
    date.getFullYear() - birth_date.getFullYear() < max_age ||
  ...
  total.innerHTML = `${weeks.length} weeks (${weeks.reduce(
    (cur, week) =>
      week[week.length - 1].getTime() <= new Date().getTime() ? cur + 1 : cur,
    0
  )} lived), ${weeks.reduce((cur, week) => cur + week.length, 0)} days`;
